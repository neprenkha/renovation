# Surau Nilai — SXX Costing Tracker V5

Supersedes:

```text
ActiveBOQ/Surau Nilai/BOQ/04_SXX_COSTING_TRACKER_V4.md
```
...
Owner corrected that prior S01 audit/takeoff format is not safe for direct paste into `*BOQ.xlsm`.

`README - BOQ.txt` requires exact paste targets and exact input columns:
...
Therefore S01 now has two valid file types:
...
Only `S01_BOQXLSM_PASTE_READY_V1.txt` should be copied into `*BOQ.xlsm`.

Do not copy the audit/reference file directly into the workbook.
