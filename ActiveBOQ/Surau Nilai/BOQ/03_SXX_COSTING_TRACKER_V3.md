# Surau Nilai — SXX Costing Tracker V3

Supersedes:

```text
ActiveBOQ/Surau Nilai/BOQ/02_SXX_COSTING_TRACKER_V2.md
```
...
S01 V1 was too blocked by missing master-list items. Owner corrected workflow: missing root master items must not stop work. Missing items must be searched/added into approved editable reference addendum under `GPTEditable/Reference_Addendum/`, then SXX must continue with clear provisional status.

S01 has been rebuilt as tab-delimited `.txt`:

```text
ActiveBOQ/Surau Nilai/BOQ/S01_Preliminaries_Costing_V2.txt
```

S01 V2 is complete enough to allow S02 as a provisional costing file.

## SXX tracker

| SXX | Proposed scope | Status | Cost file | Source evidence | Notes |
|---|---|---|---|---|---|
| S01 | Preliminaries, mobilisation, setting out, hoarding and signboard | COMPLETE FOR NEXT SXX - PROVISIONAL | `S01_Preliminaries_Costing_V2.txt` | README, Materials/Labour, GPTEditable/Reference_Addendum, BOQ control/tracker | Base working cost RM8,750.00; optional 10% contingency RM875.00; with contingency RM9,625.00. Revise after deep-read/site programme/supplier quote. |
| S02 | ESCP and temporary erosion/sediment control | NOT STARTED | - | Drawing pending deep-read | S02 may start next, but must use same SPEC A:C + Block 1-3 `.txt` order. |
...
- if root material item is missing, approved addendum reference from `GPTEditable/Reference_Addendum/` is used and status is marked;
- labour/service names and UOM match root `1.3 Labour.txt` where available;
- if root labour/service item is missing, approved addendum reference from `GPTEditable/Reference_Addendum/` is used and status is marked;
- wastage is explicit;
...
- missing master/rate/design items are listed or converted into approved provisional addendum references;
...
S01 COMPLETE FOR NEXT SXX: YES - PROVISIONAL COST
S02 START ALLOWED: YES
NEXT ACTION: Start S02 as `.txt` tab-delimited SPEC A:C + Block 1-3.
```
