# Surau Nilai — SXX Costing Tracker V9

Supersedes:

```text
ActiveBOQ/Surau Nilai/BOQ/08_SXX_COSTING_TRACKER_V8.md
```
...
## V9 one-pass S03 note

Owner instructed GPT to avoid repeating the S02 pattern and to complete each SXX in one pass wherever possible.

S03 was created with one-pass preflight before output:

```text
ActiveBOQ/Surau Nilai/Takeoff_Work/S03_ONE_PASS_PREFLIGHT_V1.md
```

## Current S03 final files

```text
Audit/reference file:
ActiveBOQ/Surau Nilai/BOQ/S03_Earthwork_Takeoff_Audit_V1.txt

Owner workbook paste file:
ActiveBOQ/Surau Nilai/BOQ/S03_BOQXLSM_PASTE_READY_V1.txt
```

Only `S03_BOQXLSM_PASTE_READY_V1.txt` should be copied into `*BOQ.xlsm`.

## Cross-SXX correction rule remains active

S03 is not hanging and may proceed to S04. If any later drawing/source/SXX proves overlap, double count, undercost or overprice, the affected SXX must be corrected immediately by creating the next cumulative version.

Watch points:

```text
S03 imported soil/backfill vs later slab/platform/build-up scopes
S03 backhoe earthwork vs S04 bridge/access excavation
S03 earthwork vs S22 permanent drainage excavation/backfill
S03 earthwork vs S24 road base/premix/parking works
```
...
| S04 | Crossing bridge and permanent site entrance | NOT STARTED | - | - | `CROSSING BRIDGE.pdf`, site plan/survey as needed | S04 may start next; must produce both audit/reference and BOQ.xlsm paste-ready output. |
